...
> 提供一个封装短信验证码的实体`study.wyy.security.sms.SmsCode`
> 提供一个发送短信的接口：`study.wyy.security.sms.SmsSender`
> 提供一个短信发送的实现类：`study.wyy.security.sms.DefalutSmsSender`
> 提供一个获取短信验证码的http接口：



> 提供一个SmsCodeAuthenticationToken，仿照`org.springframework.security.authentication.UsernamePasswordAuthenticationToken`实现即可

用于封装用户认证信息

> SmsCodeAuthenticationFilter，同样仿照`org.springframework.security.web.authentication.UsernamePasswordAuthenticationFilter`
```java
